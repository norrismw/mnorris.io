...
This variable shows that [AES](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard) encryption was used in CBC mode. The same should be used during decryption.

4. The `with open(outputFile, 'wb') as outfile:` line and the two lines following it

These lines show that the `filesize` variable (16 bytes) and the `IV` variable (also 16 bytes) are written to the output file (i.e. `enim_msg.txt`) before any encrypted data is written. As the `IV` was used in the `encryptor` variable, the same `IV` will need to be used during decryption. 
...
AES is a symmetric-key algorithm which means that the same key is used for encrypting and decrypting data.
