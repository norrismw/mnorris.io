...
Using the string from `secret.txt` as a password and usernames such as "admin", "administrator", and "floris", attempts were made to log in using the log in form on the home page, but to no avail.
...
Using the same credentials as before (`floris / Curling2018!`), the Joomla! administrator console can be accessed. From this console, there are a lot of possibilities. For example, users can be editted, templates can be modified, and extensions can be installed.
...
![protostar_default.png](/assets/images/htb/curling/protostar_default.png)protostar_default.png

Clicking on the "Protostar" link under the "Template" column shown in the image above directs to a page that allows for the files included in the template to be editted. Also, this page shows that the templates include PHP files. 
...
The `index.php` file was next editted to include the code shown below:
...
From `xxd -h`, the `-r` flag can be used to reverse an operation. Running the `xxd -r password_backup > output` creates the `output` file that contains the reuslt of the `xxd -r` command. The `file` tool was used to determine what kind of data is within the file.
